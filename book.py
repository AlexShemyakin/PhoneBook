...
class Ui_Form(QtWidgets.QWidget):
    ...
    def update_table(self):
        ...
        datab.cur.execute("SELECT name, phone, date FROM contacts")
        count = 0
        for name, phone, date in datab.cur:
            if count > 12:
            ...
            count += 1
        datab.cur.execute("SELECT name, date FROM contacts")
        timenow = datetime.datetime.now() + datetime.timedelta(days=7)
        for name, date in datab.cur:
        ...
        msg.setIconPixmap(QPixmap(os.path.join(os.getcwd(), "birth.png")).scaled(70, 70))
        msg.exec_()

    def sorting_filter(self, filt):
        self.tableWidget.clear()
        self.tableWidget.setRowCount(13)
        datab.cur.execute(filt)
        count = 0
        for name, phone, date in datab.cur:
            if count > 12:
                self.tableWidget.insertRow(count)
    ...
    def delete_contact(self):
        ...
        datab.cur.execute(f"DELETE FROM contacts WHERE name = ? AND phone = ? AND date = ?", (name, phone, date))
        datab.conn.commit()
