...
class Ui_Form(QtWidgets.QWidget):
    ...
    def add_contact(self):
        ...
        try:
            datab.cur.execute(f"SELECT name FROM contacts WHERE name = '{self.name.text()}' "
                              f"and phone = '{self.number_phone.text()}'"
                              f" and date = '{self.date.text()}'")
            select_name = datab.cur.fetchone()[0]
            datab.cur.execute(f"SELECT phone FROM contacts WHERE name = '{self.name.text()}' "
                              f"and phone = '{self.number_phone.text()}'"
            ...
            select_phone = str(datab.cur.fetchone()[0])
            datab.cur.execute(f"SELECT date FROM contacts WHERE name = '{self.name.text()}' "
                              f"and phone = '{self.number_phone.text()}'"
                              f" and (SELECT DATE_FORMAT(date, '%Y-%m-%d') FROM contacts WHERE name = '{self.name.text()}' "
                              f"and phone = '{self.number_phone.text()}') = '{self.date.text()}'")
            select_date = datab.cur.fetchone()[0]
        except:
            pass

        if self.name.text() != select_name and self.number_phone.text() != select_phone and self.date.text() != select_date:
            datab.add_data(self.name.text(), self.number_phone.text(), self.date.text())
            msg = QMessageBox()
