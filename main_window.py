...
class window(QtWidgets.QWidget):
    ...
    def login(self):
        datab.cur.execute(f"SELECT name, password FROM users WHERE name = '{self.lineEdit.text()}' "
                          f"AND password = '{self.lineEdit_2.text()}'")
        count = 0
        for name, password in datab.cur:
            if self.lineEdit.text() == name and self.lineEdit_2.text() == password:
                count = 1
                self.close()
                self._new = book.Ui_Form()
                self._new.show()
        if count == 0:
            msg = QMessageBox()
    ...
    def auto_login(self):
        if self.checkBox.isChecked() == True:
            datab.cur.execute(f"UPDATE users SET remember_me = 1 WHERE name = '{self.lineEdit.text()}' "
                              f"AND password = '{self.lineEdit_2.text()}'")
            datab.conn.commit()
            datab.cur.execute(f"UPDATE users SET remember_me = 0 WHERE name <> '{self.lineEdit.text()}' "
                              f"AND password <> '{self.lineEdit_2.text()}'")
            datab.conn.commit()
    ...
    def setupUi(self, Form):
        ...
        datab.cur.execute("SELECT name, password, remember_me FROM users WHERE remember_me = '1'")
        for name, password, remember_me in datab.cur:
            if remember_me == 1:
